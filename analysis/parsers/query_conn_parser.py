...
# Constants
COLUMNS = ["timestamp", "request_id", "dbname", "backlog", "latency"]
QUERY_CONN_LOG_PATTERN = r"^\[([0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}\.[0-9]+)\] pid=([^\s]+) tid=([^\s]+) request_id=([^\s]+) dbname=([^\s]+) backlog=([^\s]+) latency=([^\s]+)$"


class QueryConnParser:
    @classmethod
    def df(cls, logfile):
    ...
    def extract_values_from_log(log):
        match = re.match(QUERY_CONN_LOG_PATTERN, log)
        if not match:
            return None
        timestamp, _, _, request_id, dbname, backlog, latency = match.groups()
        return (timestamp, request_id, dbname, backlog, latency)
...
if __name__ == "__main__":
    ...
    with open(args.log_filepath) as logfile:
        QueryConnParser.df(logfile).to_csv(args.csv_filepath, index=False)
