...
# Constants
COLUMNS = ["timestamp", "cpu", "qlen"]
RUNQLEN_LOG_PATTERN = r"^([0-9\.\-\:]+)\s+(\d+)\s+(\d+)$"
...
class RunqlenParser:
    ...
    def extract_values_from_log(log):
        match = re.match(RUNQLEN_LOG_PATTERN, log.strip())
        if not match:
            return None
        timestamp, cpu, qlen = match.groups()
        return (timestamp, cpu, qlen)
...
if __name__ == "__main__":
    ...
    with open(args.log_filepath) as logfile:
        RunqlenParser.df(logfile).to_csv(args.csv_filepath, index=False)
