...
# Constants
COLUMNS = ["timestamp", "request_id", "service_name", "command", "latency"]
REDIS_LOG_PATTERN = r"^\[([0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}\.[0-9]+)\] pid=([^\s]+) tid=([^\s]+) request_id=([^\s]+) latency=([^\s]+) service_name=([^\s]+) command=([^\s]+)$"
...
class RedisParser:
    ...
    def extract_values_from_log(log):
        match = re.match(REDIS_LOG_PATTERN, log)
        if not match:
            return None
        timestamp, _, _, request_id, latency, service_name, command = match.groups()
        return (timestamp, request_id, service_name, command, latency)
...
if __name__ == "__main__":
    ...
    with open(args.log_filepath) as logfile:
        RedisParser.df(logfile).to_csv(args.csv_filepath, index=False)
