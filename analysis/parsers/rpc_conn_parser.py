...
# Constants
COLUMNS = ["timestamp", "request_id", "service", "backlog", "latency"]
RPC_CONN_LOG_PATTERN = r"^\[([0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}\.[0-9]+)\] pid=([^\s]+) tid=([^\s]+) request_id=([^\s]+) service=([^\s]+) backlog=([^\s]+) latency=([^\s]+)$"
...
class RPCConnParser:
    ...
    def extract_values_from_log(log):
        ...
        if not match:
            return None
        timestamp, _, _, request_id, service, backlog, latency = match.groups()
        return (timestamp, request_id, service, backlog, latency)
